...
class NamespaceUseStatementOrderSniff extends Sniff {
	...
	public function process_token( $stackPtr ) {
		 ...
		 *
		 * If the order has to be changed too, the fixers here will run in a subsequent loop.
		 */
	...
	/**
	 * Check for a blank line after the last statement of the last block.
	 *
	 * @param array  $statements Array with start/end pointers of each use statement in a group.
	 * @param string $type       Use statement group type.
	 */
	protected function check_blank_line_after_group( $statements, $type ) {
		...
		$error = 'There must be exactly one blank line after the last statement in a "use %s" statement group.';
		$data  = array( $type );
